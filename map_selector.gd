...
func _on_mouse_area_mouse_exited():
	var tween = create_tween()
	tween.tween_property($Bubble, "scale", Vector2(0, 0), 0.2).set_trans(Tween.TRANS_EXPO).set_ease(Tween.EASE_IN)
	await get_tree().create_timer(0.2).timeout


func _on_mouse_area_input_event(viewport, event, shape_idx):
